...
app.post('/api/tc-sorgula', (req, res) => {
  ...
  for (const [hastaneId, h] of Object.entries(VERI)) {
    const q = h.queues.find(x => x.tc === tc && !x.called);
    if (q) return res.json({ bulundu: true, hastaneId, num: q.num, acil: q.acil, oyku: q.oyku });
  }
...
function getOzet() {
  ...
  for (const [id, h] of Object.entries(VERI)) {
    const total = h.counts.green + h.counts.yellow + h.counts.red;
    ozet[id] = { name: HASTANE_KIMLIKLER[id].name, fill: Math.min(100, Math.round((total / h.capacity) * 100)), capacity: h.capacity, counts: h.counts, siraCount: h.queues.filter(q=>!q.called).length };
  }
  return ozet;
}
