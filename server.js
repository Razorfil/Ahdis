...
app.post('/api/hasta-ekle', (req, res) => {
  ...
  // Sıra numarasını mevcut counts değerine göre ver
  const base = { green: 1000, yellow: 3000, red: 5000 };
  const max  = { green: 2999, yellow: 4999, red: 6999 };
  const mevcutSira = VERI[hastaneId].counts[acil]; // panelden ayarlanan kişi sayısı
  let num = base[acil] + mevcutSira;
...
// TC sorgula — önündeki kişi sayısını da döndürür
app.post('/api/tc-sorgula', (req, res) => {
  ...
  for (const [hastaneId, h] of Object.entries(VERI)) {
    ...
    if (q) {
      // Önündeki kişi: aynı aciliyet, daha küçük sıra numarası, henüz çağrılmamış
      const onunde = h.queues.filter(x => x.acil === q.acil && x.num < q.num && !x.called).length;
      return res.json({ bulundu: true, hastaneId, num: q.num, acil: q.acil, oyku: q.oyku, called: q.called, onunde });
    }
  }
